...
## 2026-08-28：账号登录、人机验证与演示数据补全

- 员工端新增 `staff / 88888888` 登录，HR 演示账号统一为 `admin / 88888888`。
- 两类登录共用服务端一次性滑动拼图验证，题目 5 分钟过期且验证后立即失效。
- 员工问答、会话和意见接口要求员工 Session，并按员工账号使用稳定的数据归属标识。
- `flask seed-demo-data` 幂等补齐员工查询、回答状态、意见投递及 HR 处理时间线；`setup-demo.ps1` 已自动调用。
- 员工端增加个人查询/意见数据概览，HR 端原有统计、意见队列与处理进度直接展示样例数据。
- 后续业务代码继续只在 `04_源代码` 内修改。
