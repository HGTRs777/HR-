...
- 员工端加入按办理事项逐项反问的条件收集链路，情景沙盘改为动态所需条件表，最终输出带可点击制度依据的办理清单；旧版“一问一办”仅保留后端兼容字段，不再作为产品功能或页面入口。
- HR 端新增制度漏洞扫描批次和漏洞项持久化，定期结合启用制度、匿名问答聚合与员工反馈执行 AI 审计；模型不可用时自动降级为规则分析，并支持手动立即扫描。
- 新增迁移 `d2a79e3b9f10`，迁移检查无漂移；后端 Pytest 40 项、前端 Vitest 9 项、TypeScript strict 与 Vite 生产构建全部通过。
- 员工端后续完成信息架构重排：可折叠历史对话侧栏承载新增/切换/删除会话，主区改为聊天与“情景沙盘与办理清单”双列；意见投递和制度依据改为按需弹窗。组件回归增至 9 项并通过。
- 办理清单调整为纸张式任务卡，支持逐项勾选、完成数/总数、预计完成日期、流程展开和清单导出；历史会话标题移除“[演示]”前缀，并在悬停时横向滚动展示长标题。演示数据命令会同步兼容清理旧前缀。
- HR 治理首页加入图标式模块跳转导航，反馈闭环迁入顶部“意见处理”独立视图，制度漏洞扫描依据改为结构化字段卡；索引发布请求单独延长到 180 秒并加入长任务提示，Top 5 明确为发布后的高级检索验收工具。
- 员工意见投递由弹窗改为独立界面；实名意见自动采用登录账号身份。演示员工显示为“陈晨 · 工号 E1001”，演示 HR 显示为“李娜 · 工号 HR1001”。
- 员工意见表单增加标签与输入区间距。HR 意见处理入口改为与员工意见投递一致的描边按钮并展示未处理数量；模块导航加入滚动位置监听。前三个治理入口合并为“制度管理”，索引操作更名为“更新制度索引”。Top 5 结果在验证区下方给出合格结论并与制度版本卡底部齐平，完整诊断表迁入按需弹窗。
...
- 后续任务不得删减 F01–F18，不得扩展到完整 HRIS、考勤审批或薪资管理；问答所需最小员工业务档案已经纳入当前范围。
...
- 新配置：`DEEPSEEK_MAX_OUTPUT_TOKENS`、`DEEPSEEK_MAX_RETRIES`、`DEEPSEEK_RETRY_BACKOFF_SECONDS`、`RETRIEVAL_MIN_VECTOR_SCORE`、`CLAIM_EVIDENCE_MIN_SCORE`。全部 DeepSeek 阶段复用同一 client；临时 HTTP、超时、连接及结构化输出异常最多重试 2 次。
